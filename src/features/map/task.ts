...
const {LOCATION_STATE_CHANGED, CURRENT_LOCATION, LAST_BACKGROUND_PROCESS, LAST_FOREGROUND_PROCESS} = ASYNC_STORAGE_KEYS;

interface TLocationTaskData {
  locations: Location.LocationObject[];
}

const handleFastMovingVisit = async (location: Location.LocationObject): Promise<void> => {
  try {
...
const syncForegroundLocationToBackend = async (location: Location.LocationObject): Promise<void> => {
  try {
    ...
    if (results && results.length > 0) {
      ...
      console.log('🔄 Synced foreground location to backend')
    }
  } catch (error) {
...
TaskManager.defineTask(
  ...
  async ({data, error}: TaskManager.TaskManagerTaskBody<TLocationTaskData>) => {
    if (error) {
    ...
    if (data) {
      ...
      try {
        ...
        if (timeSinceLastProcess < requiredInterval) {
          console.log(`⏱️ Skipping update - only ${Math.round(timeSinceLastProcess / 1000)}s passed, need ${Math.round(requiredInterval / 1000)}s`)
          return;
        }
        ...
        const stateResult = await userLocationStateService.determineState(location);
        const newState = stateResult.newState;

        console.log(`🏃 Current state: ${newState} (v: ${stateResult.velocity?.toFixed(2)} km/h)`)
